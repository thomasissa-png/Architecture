...
function buildSurfacesResponsesPrompt(surfacePrompt: string): string {
  return [
    ...
    "Preserve the ceiling geometry exactly — vaults, beams, ribs, arches, and exposed structural elements must remain visible with their original rough texture, irregular edges, and surface patina intact. Apply the finish OVER the existing geometry. Do NOT smooth, flatten, or clean up beams or structural features. No visible joint tape, no screw dimples, no raw plasterboard seams — the ceiling must appear fully finished and smooth between structural elements.",
    "Remove all visible construction elements: dangling cables, exposed wiring, junction boxes without covers, cable conduits, and temporary fixtures. These are construction leftovers, not permanent equipment.",
...
function buildSurfacesFluxPrompt(surfacePrompt: string): string {
  return [
    ...
    "Preserve ceiling geometry — vaults, beams, ribs, arches remain visible with original rough texture and edges. Apply finish over existing structure, do not smooth or flatten. No visible joint tape, screw dimples, or raw plasterboard seams — ceiling must appear fully finished.",
