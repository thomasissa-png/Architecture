...
export async function POST(
  ...
  try {
    ...
    if (!apiKey || apiKey === "..." || apiKey.startsWith("sk_test_")) {
      ...
      return NextResponse.json({
        lots: [{ lot_name: "Lot 1", lot_type: "appartement", room_ids: [] }],
        source: "fallback",
      });
    }
    ...
    const systemPrompt = `You are an expert real estate analyst. You are looking at a floor plan image. Your job is to identify how many separate residential/commercial units (lots/biens) are visible on this plan.
...
Return a JSON object with this exact structure:
{
  ...
  "reasoning": "Brief explanation of why you identified these lots"
    ...
    let parsed: { lots?: Array<{ lot_name: string; lot_type?: string }> };
    try {
