...
export const OUTDOOR_STYLES: Record<string, OutdoorStyle> = {
  ...
  cosy_balcon: {
    ...
    furniturePrompt:
      "A tiny open-air nook — vary furniture and plants each generation. COMPACT items only for narrow balcony. Table (choose one: round zinc-top bistro 60cm, small square teak 50cm, round mosaic tile-top 55cm), seating (choose one: two folding metal chairs matt black with cream pads, two rattan bistro chairs with cushions, one small wooden bench 90cm with pad). Tall planter 25x25x70cm (choose one: Trachelospermum jasminoides trailing, Clematis on small trellis, Plumbago with blue flowers). Rail planter 60x15cm (choose one: Hedera helix trailing, Pelargonium in mixed colors, Lobelia cascading). Battery LED string lights 2200K along rail (unlit daytime). Table accent (choose one: small ceramic lantern 15cm with candle, tiny potted herb, beeswax candle in terracotta cup). Intimate atmosphere. Leave 60cm clear passage.",
  },
};
