...
const CLEANUP_V53 = "REMOVE only these TEMPORARY construction items: any people visible (workers, painters, occupants, photographers, hands, arms, silhouettes), loose cables, exposed pipes lying on floor, open wiring, construction debris, rubble, plaster dust, cardboard boxes, paint pots, buckets, tarps, drop cloths, ladders, step-stools, scaffolding, loose hand tools, loose power tools, brooms, work gloves. For each removed item, fill the vacated zone with the surrounding wall, floor, or ceiling finish. HIDE OR REMOVE if aesthetically inappropriate: water heaters, boilers, exposed gas pipes. If removal is impossible (still in frame), cover the zone with the surrounding wall finish or leave as a clean recess. PRESERVE all PERMANENT wall-mounted or built-in equipment: electrical panels, fuse boxes, circuit breakers, thermostats, light switches, wall outlets in their recessed housing, radiators, convectors, heaters, vents, towel dryers. Preserve all existing built-in sanitary fixtures if present (bathtub, shower tray, toilet, sink). The output MUST be rendered empty of all people, tools, and debris. The output MUST show zero humans, zero hands, zero arms, zero silhouettes of people — this is a listing photo for a real-estate portal. No furniture, no new fixtures, no decorative items.";
