...
export default function DossierResult({
  ...
  return (
    <div className="space-y-6" data-testid="dossier-result">
      {/* Summary bar */}
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl border border-[var(--border)] bg-white/40">
        <div>
          <h3 className="text-sm font-semibold text-[var(--foreground)]">
            {bienNom}
          </h3>
          <p className="text-xs text-[var(--muted)] font-light mt-0.5">
            {completedPhotos.length} visualisation{completedPhotos.length > 1 ? "s" : ""} generee{completedPhotos.length > 1 ? "s" : ""}
            {failedPhotos.length > 0 && ` — ${failedPhotos.length} echec${failedPhotos.length > 1 ? "s" : ""}`}
          </p>
        </div>
          ...
          >
            ...
            <div className="px-4 py-2.5 border-b border-[var(--border)] flex items-center justify-between">
              ...
              {onRegenerate && (
                ...
                >
                  {isRegenerating === photo.id ? "Regeneration..." : "Regenerer"}
                </button>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2">
              ...
              <div className="relative">
                ...
                <span className="absolute bottom-2 left-2 text-[10px] font-medium text-gray-400 bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded-full">
                  AVANT
                </span>
              </div>
              ...
              <div className="relative">
                ...
                <span className="absolute bottom-2 left-2 text-[10px] font-medium text-[var(--sage)] bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded-full">
                  APRES
                </span>
              </div>
            </div>
      ...
      {failedPhotos.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm text-red-400 font-medium">
            {failedPhotos.length} photo{failedPhotos.length > 1 ? "s" : ""} en echec
          </p>
          {failedPhotos.map((photo) => (
            ...
            >
              ...
              {onRegenerate && (
                ...
                >
                  {isRegenerating === photo.id ? "..." : "Regenerer (1 credit)"}
                </button>
              )}
            </div>
          ))}
