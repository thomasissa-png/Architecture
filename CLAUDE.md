...
- **REGLE PROPAGATION CROSS-HANDLER (session 34)** : tout fix sur un handler de fetch/reseau (`handleGenerate`, `handleRefine`, `handleRegenerate`, etc.) declenche obligatoirement un audit des handlers voisins du meme fichier. Les regressions silencieuses viennent toujours de fixes non propages. Checklist post-fix reseau : (1) `grep -nE "fetch\(|resilientFetch\(" app/page.tsx components/*.tsx` pour lister tous les handlers, (2) verifier que le meme pattern de catch (`BackgroundDisconnectError`, refund credit, toast galerie) est applique partout, (3) verifier que chaque consumer de credit a un refund automatique sur erreur. Exemple : session 34 BR-4, le fix session 32 sur `handleGenerate` n'avait jamais ete audite sur `handleRefine`/`handleRegenerate` → 2 BR P0 sur du code "deja corrige".
- **REGLE STATE INDEXE PAR ELEMENT DE LISTE (session 34)** : tout state lie a une operation sur un element d'une liste DOIT etre indexe par l'identifiant de l'element (`Set<number>` ou `Map<number, X>`), JAMAIS un single state global. Anti-pattern : `isRefining: boolean` + `refiningIndex: number` dans le meme composant → migrer vers `refiningIndices: Set<number>`. Exemple : session 34 BR-3, migration `isRefining` single state → `refiningIndices: Set<number>` + `secondaryAbortByIndexRef: Map<number, AbortController>`. A auditer dans tout composant qui gere des collections (galerie, dossiers, mes-biens).
- **REGLE LABELS POST-PARALLELISATION (session 34)** : tout label de progression DOIT deriver de l'etat reel des donnees, JAMAIS d'un compteur sequentiel herite d'une epoque single-threaded. Anti-pattern : `{currentIndex + 1}/{total}` quand les jobs tournent en parallele. Pattern correct : `doneCount = results.filter(r => !r.pending).length` + affichage adaptatif (0 done / some done / all done). Quand on parallelise un traitement (`MAX_CONCURRENT > 1`), grep tous les labels contenant `${index}/${total}` ou `progress`. Exemple : session 34 BR-1.
- **REGLE LOADING STATE CASCADE (session 34)** : tout `<img src>` dans un loading state DOIT pouvoir afficher un resultat partiellement disponible. Le fallback "input image" est le DERNIER recours, pas le defaut. Pattern correct : cascade `done > partial > input` (`const src = done ? doneResult.url : partial ? partialResult.url : input.url`). Exemple : session 34 BR-2, surface-only photo terminait pendant que les autres etaient en passe 2 → tile affichait l'input au lieu du pass1 resultat.
- **REGLE MOCKS VITEST AVEC `new` (session 34)** : quand un mock stubbe une classe utilisee avec `new` (ex : `new OpenAI()`, `new Client()`), TOUJOURS utiliser une vraie `class` ou une `function` traditionnelle, JAMAIS arrow function. Anti-pattern : `vi.fn().mockImplementation(() => ({ ... }))` → jette `TypeError: ... is not a constructor`. Pattern correct : `class OpenAI { chat = { completions: { create: chatCreateMock } }; constructor() {} }`. Triage : un mock qui jette "is not a constructor" est TOUJOURS un probleme de mock, pas de code metier. Vitest 4 warning : `"did not use 'function' or 'class' in its implementation"`.
- **REGLE 100% TESTS PASS (session 34, preference fondateur)** : tout `npx vitest run` qui termine avec `failed > 0` BLOQUE le passage a la tache suivante. Les `skipped` sont OK uniquement si intentionnels et commentes avec raison explicite. JAMAIS de "TODO plus tard" sur un test failing, meme si le bug est dans le test et pas le code.
- **REGLE COVERAGE-DRIVEN REFACTOR (session 34)** : des lors qu'une couverture tests >= 80% est en place, un refactor structurel n'est plus "risque" — il est mesurable et reversible. Le vrai risque de refactor = absence de tests, pas le code lui-meme. Un agent qui refuse un refactor doit documenter un OBSTACLE TECHNIQUE PRECIS (ex : "R1 manque le callback X"), PAS une excuse floue ("risque regression"). Exemple : session 34 round 3, @fullstack a refuse de cabler R1+R2 dans `handleGenerate` en invoquant "risque regression > valeur" ; round 5, @fullstack a reussi le meme cablage en 2 commits atomiques avec zero regression grace aux 175 tests en place. Citation : "L'extraction etait bien concue des l'origine ; le seul obstacle etait la frilosite — frilosite levee par les tests qui detectent immediatement toute regression." Les tests sont le FILET de securite qui rend le refactor sur. Jamais invoquer "paresse technique" quand on a un filet de securite.
