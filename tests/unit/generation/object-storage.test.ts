...
let nextClientFails = false;
let storedBlobs = new Map<string, Buffer>();
let clientCallCount = 0;
let internalState = "ready";
...
beforeEach(() => {
  ...
  nextClientFails = false;
  internalState = "ready";
  // Reset storage singleton — re-import lib/db forces a fresh module
  vi.resetModules();
...
describe("G6 — saveImage / getImage", () => {
  it("U-DB-001: saveImage upload OK + verify OK → retourne key", async () => {
    ...
    expect(storedBlobs.has("logs/test1.jpg")).toBe(true);
  });
  ...
  it("U-DB-001b: saveImage stocke le buffer décodé en base64", async () => {
    ...
    const stored = storedBlobs.get("logs/test2.jpg");
  ...
  it("U-DB-003: withStorageRetry réinit client sur échec puis réussit", async () => {
    ...
    nextClientFails = true; // first call throws
